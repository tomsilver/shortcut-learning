...
class Obstacle2DEnv(gym.Env):
    ...
    def _get_default_state(self) -> Obstacle2DState:
        """Get default initial state with continuous random positions.

        This is the "hard" version with diverse initial states for better training data.
        Randomizes target area position, robot position, and both block positions.
        The target area position is included in observations so policies can see it.
        """
        # Randomize target area position (keeping width/height fixed)
        target_width = self._target_area["width"]
        target_height = self._target_area["height"]
        # Target area center can be anywhere that keeps it within bounds [0, 1]
        # With width=0.2, center x can be in [0.1, 0.9]
        target_x = self.np_random.uniform(target_width / 2, 1.0 - target_width / 2)
        # Keep target at y=0.0 (on the ground)
        target_y = self._target_area["y"]

        # Update the target area for this episode
        self._target_area["x"] = target_x

        target_left = target_x - target_width / 2
        target_right = target_x + target_width / 2

        # Use continuous uniform distributions for diverse training data
        # Block 2 can be anywhere in the target area (fully inside)
        block_2_x = self.np_random.uniform(target_left, target_right)

        # Block 1 outside target area (avoid overlaps)
        # Block width is 0.2, so we need margin to avoid overlap
        block_width = 0.2

        # Calculate safe regions: blocks must be fully outside target
        # Left region: [block_width/2, target_left - block_width/2]
        # Right region: [target_right + block_width/2, 1.0 - block_width/2]
        left_min = block_width / 2
        left_max = target_left - block_width / 2
        right_min = target_right + block_width / 2
        right_max = 1.0 - block_width / 2

        left_valid = left_max > left_min
        right_valid = right_max > right_min

        if left_valid and right_valid:
            # Both regions available, choose randomly
            if self.np_random.random() < 0.5:
                block_1_x = self.np_random.uniform(left_min, left_max)
            else:
                block_1_x = self.np_random.uniform(right_min, right_max)
        elif left_valid:
            block_1_x = self.np_random.uniform(left_min, left_max)
        elif right_valid:
            block_1_x = self.np_random.uniform(right_min, right_max)
        else:
            # Fallback: target takes up most of space, place as far from target as possible
            if target_x < 0.5:
                block_1_x = 0.9  # Far right
            else:
                block_1_x = 0.1  # Far left

        # Robot can start anywhere horizontally (was fixed at 0.5)
        robot_x = self.np_random.uniform(0.0, 1.0)

        return Obstacle2DState(
            robot_position=np.array([robot_x, 1.0], dtype=np.float32),
