...
from shortcut_learning.problems.obstacle2d_hard.env import (
    Obstacle2DEnv,
...
class PutDownSkill(BaseObstacle2DSkill):
    ...
    def _get_action_given_objects(
        ...
        if surface_obj.name == "target_area":
            ...
            # Put on table, away from target area
            # Use target area position to decide where to drop
            target_area_x = obs[11]
            target_area_width = obs[13]

            # If target is on left half, drop on right; if on right half, drop on left
            if target_area_x < 0.5:
                # Target on left, drop on right side
                target_x = 0.85
            else:
                # Target on right, drop on left side
                target_x = 0.15
            target_y = 0.0
