...
class DQNHeuristicWrapper(gym.Wrapper):
    ...
    This wrapper modifies the reward structure to be -1 per step,
    regardless of whether the goal is reached. This causes the learned
    value function V(s, z') to approximate -distance(s, z'), where s is
    the low-level state and z' is the abstract goal state (atoms).
    """
    ...
    def step(
        ...
        if len(self._reward_history) <= 10:
            print(
                f"[DEBUG REWARD STEP {len(self._reward_history)}] reward={reward:.1f}, "
        ...
        if terminated or truncated:
            ...
            if self._episode_count <= 3:
                ...
                print(
                    f"[DEBUG EPISODE {self._episode_count}] "
...
class DQNHeuristic(BaseHeuristic):
    ...
    Trains a policy with reward=-1 per step, causing the value function
    to learn V(s, g) ≈ -distance(s, g). The heuristic can then estimate
    distances by querying the critic/value network.
    """
    ...
    def __init__(
        ...
        print(
            f"Using algorithm: {algorithm} for action space: {type(action_space).__name__}"
        )
    ...
    def train(
        self, state_node_pairs: list[tuple[ObsType, int]], num_steps: int
    ) -> None:
    ...
    def multi_train(
        ...
        print(f"\n{'='*80}")
        print(
            f"MULTI-ROUND TRAINING: {num_rounds} rounds, {num_steps_per_round} steps/round"
        )
        ...
        for round_idx in range(num_rounds):
            ...
            self.train(
                state_node_pairs=current_state_pairs, num_steps=num_steps_per_round
            )
    ...
    def load(self, path: str, perceiver: Any) -> None:
        ...
        if self.algorithm_used == "custom_dqn":
            ...
            if self.algorithm_used == "SAC":
                self.model = SAC.load(
                    f"{path}/distance_model", env=dummy_env, device=self.device
                )
            elif self.algorithm_used == "DQN":
                self.model = DQN.load(
                    f"{path}/distance_model", env=dummy_env, device=self.device
    ...
    def _compute_obs_statistics(
        self, state_pairs: list[tuple[ObsType, ObsType]]
    ) -> None:
        ...
        print(
            f"Computed observation statistics: mean={self._obs_mean.mean():.3f}, std={self._obs_std.mean():.3f}"
        )
