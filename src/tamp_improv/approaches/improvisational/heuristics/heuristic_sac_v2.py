...
#                   achieved_atoms_vec, next_achieved_atoms_vec, obs_raw, base_action)
_TrajStep = tuple[NDArray, NDArray, float, NDArray, bool, NDArray, NDArray, Any, NDArray | None]
...
class SACReplayBuffer:
    ...
    def sample(
        self, batch_size: int
    ) -> tuple[NDArray, NDArray, NDArray, NDArray, NDArray, NDArray, list, NDArray, NDArray, NDArray | None]:
...
class SACv2Heuristic(BaseHeuristic):
    ...
    def train_one_round(self, checkpoint_callback: Any = None) -> dict[str, Any]:
        ...
        for epoch in range(self.config.num_epochs_per_round):
            ...
            _t_collect = _time.time() - _t_collect

            # Update networks
            _t_update = _time.time()
            ...
            if (epoch + 1) % 1 == 0 or epoch == 0:
                ...
                print(
                    ...
                    f"collect: {_t_collect:.1f}s | update: {_t_update:.1f}s"
                )
    ...
    def _add_to_buffer_with_her(
        ...
        for step_data in trajectory:
            ...
            ba = step_data[8] if len(step_data) > 8 else None
    ...
    def _update_networks(self) -> tuple[float, float]:
        ...
        if _do_print:
            print(f"  [timing] critic_update: {_time.time()-_t_section:.3f}s", flush=True)
        ...
        if _do_print:
            print(f"  [timing] actor_update: {_time.time()-_t_section:.3f}s", flush=True)
    ...
    def _batch_estimate_raw_distances(self, pairs: list[tuple[int, int]]) -> np.ndarray:
        ...
        the Q-based distance estimates across them to reduce variance.
        Returns array of shape (len(pairs),) with raw distances (dist_scale=1).
        """
        device = torch.device(self.config.device)
        k = self.config.node_distance_samples

        # Build batched inputs: k samples per pair → total batch size = len(pairs) * k
        states_list = []
        goals_list = []
        ...
        raw_all = np.maximum(0.0, -q.cpu().numpy().squeeze(-1))  # shape (total_batch,)
    ...
    def exact_gain(self, source_node: int, target_node: int, L: float | None = None) -> float:
        ...
        debug = getattr(self, "_debug_exact_gain", True)
        if debug:
            d_xy_direct = d[x, y]
            print(
    ...
    def _update_gains(self) -> None:
        ...
        if self.config.auto_dist_scale:
            ...
            print("Raw dists:", raw_dists)
            print("Original dists:", orig)
            print("Ratios:", ratios)
